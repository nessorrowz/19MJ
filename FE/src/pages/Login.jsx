...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(data.user.role === 'company' ? '/dashboard/company' : '/dashboard');
    } catch (err) {
      triggerShake();
      ...
      <div className="auth-left">
        <form className={`auth-card ${shake ? 'shake' : ''}`} onSubmit={handleSubmit} noValidate>
          ...
          <div className="bottom-link">
            Belum punya akun?{' '}
            <Link to="/register/candidate">Daftar sebagai Kandidat</Link>
            {' · '}
            <Link to="/register/company">Daftar sebagai Perusahaan</Link>
          </div>
        </form>
      </div>
